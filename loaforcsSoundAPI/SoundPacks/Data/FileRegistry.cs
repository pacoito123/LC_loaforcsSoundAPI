...
public class FileRegistry<T>(SoundPack pack, string relativePath) : Registry<T, List<T>>() where T : class, IFilePathAware {
    ...
    internal void EnableHotReload() {
        ...
        void UnloadOldFile(string fullPath) {
            T existing = FindValue(it => it.FilePath == fullPath);
            if(existing != null) {
