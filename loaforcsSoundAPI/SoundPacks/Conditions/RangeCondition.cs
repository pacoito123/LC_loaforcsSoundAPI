...
public abstract class RangeCondition<T> : Condition where T : struct, IComparable<T> {
    ...
    /// <inheritdoc cref="ValidateRangeOperator(string,out RangeOperator&lt;T&gt;,out IValidatable.ValidationResult)"/>
    ...
    protected bool ValidateRangeOperator(string condition, out RangeOperator<T> range, out IValidatable.ValidationResult result) {
        ...
        switch(parts.Length) {
            case 1:
                ...
                T target = range.min;
                if(!TryParseValue(parts[0], ref target)) {
                    // Invalid input.
            ...
            case 2:
                ...
                if(!TryParseValue(parts[0], ref lowerBound)) {
                    // Invalid input.
                    result = new IValidatable.ValidationResult(IValidatable.ResultType.FAIL, $"Failed to parse: '{parts[0]}' as a '{typeof(T).FullName}'!");
                    break;
                }
                ...
                if(!TryParseValue(parts[1], ref upperBound)) {
                    // Invalid input.
